...
let region = 65; // Свердловская
let csv_url = 'getCardsCSV';
let xml_url = 'getCardsXML';
let file_ext = 'xml';

process.chdir(path.join('D:', 'code', 'Python', 'data_xml'));
...
async function download_file(year, month, r) {
  ...
  const response = await fetch(`http://stat.gibdd.ru/${xml_url}`, {
  "headers": {
    "accept": "*/*",
  ...
  await new Promise((resolve, reject) => {
    ...
    fileStream.on("finish", function() {
      ...
      frs.on('close', ()=> {
        fs.rename(`Карточки ДТП.${file_ext}`,`${start_date}.${file_ext}`, ()=> {
...
///todo обработка ошибок (неправильный JSON) 